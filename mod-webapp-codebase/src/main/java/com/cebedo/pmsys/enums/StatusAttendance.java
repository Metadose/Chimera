...
public enum StatusAttendance {
    ...
    StatusAttendance(int idn) {
	this.id = idn;
    }

    StatusAttendance(int idn, String lbl, String cssClass) {
	this.label = lbl;
	this.id = idn;
	this.css = cssClass;
    }
	...
	for (StatusAttendance stat : StatusAttendance.class.getEnumConstants()) {
    ...
    public static StatusAttendance of(int idn) {
	if (idn == PRESENT.id()) {
	    return PRESENT;
