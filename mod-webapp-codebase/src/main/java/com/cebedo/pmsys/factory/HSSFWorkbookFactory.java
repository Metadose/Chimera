package com.cebedo.pmsys.factory;
...
public class HSSFWorkbookFactory {
	...
	addRowHeader(sheetName, IndexedColors.YELLOW, StatusAttendance.OVERTIME.label(), "Count");
	addRowsStaffAttendanceCount(sheetName, statisticsStaff, StatusAttendance.OVERTIME, max, order);

	addRowEmpty(sheetName);
	addRowHeader(sheetName, IndexedColors.YELLOW, StatusAttendance.LATE.label(), "Count");
	addRowsStaffAttendanceCount(sheetName, statisticsStaff, StatusAttendance.LATE, max, order);

	addRowEmpty(sheetName);
	addRowHeader(sheetName, IndexedColors.YELLOW, StatusAttendance.HALFDAY.label(), "Count");
	addRowsStaffAttendanceCount(sheetName, statisticsStaff, StatusAttendance.HALFDAY, max, order);

	addRowEmpty(sheetName);
	addRowHeader(sheetName, IndexedColors.YELLOW, StatusAttendance.LEAVE.label(), "Count");
	addRowsStaffAttendanceCount(sheetName, statisticsStaff, StatusAttendance.LEAVE, max, order);
    }
    ...
    private void addRowsStaffAttendanceCount(String sheetName, StatisticsStaff statisticsStaff,
	    StatusAttendance attendanceStatus, int max, SortOrder order) {
	...
	for (EstimateCost cost : list) {
	    ...
	    // Data.
	    double costValue = subType == TypeEstimateCost.SUB_TYPE_PLANNED ? cost.getCost()
		    : cost.getActualCost();
	    String phase = subType == TypeEstimateCost.SUB_TYPE_PLANNED ? "Estimated" : "Actual";
	...
	String subTypeText = subType == TypeEstimateCost.SUB_TYPE_ABSOLUTE ? "ABSOLUTE DIFFERENCE"
		: "DIFFERENCE";

	// Data.
	ImmutableList<Entry<EstimateCost, Double>> direct = subType == TypeEstimateCost.SUB_TYPE_ABSOLUTE
		? statEstimates.getSortedAbsDiffDirect(limit, order)
		: statEstimates.getSortedDifferencesDirect(limit, order);

	ImmutableList<Entry<EstimateCost, Double>> indirect = subType == TypeEstimateCost.SUB_TYPE_ABSOLUTE
		? statEstimates.getSortedAbsDiffIndirect(limit, order)
		: statEstimates.getSortedDifferencesIndirect(limit, order);

	ImmutableList<Entry<EstimateCost, Double>> overall = subType == TypeEstimateCost.SUB_TYPE_ABSOLUTE
     ...
     */
    public void addRowsTaskCount(String sheetName, Map<StatusTask, Integer> taskStatusCountMap,
	    int tasksPopulation) {
	for (StatusTask status : taskStatusCountMap.keySet()) {
