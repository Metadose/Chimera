...
public class WAEngine extends WAQueryParametersImpl {
    ...
    // These fields belong to the engine. They are not propagated to the
    // WAQuery. If you want different values
    // for any of these in your app, create a differnt WAEngine. You can execute
    // WAQuery objects created by one
    // WAEngine in another.
    private String server = "api.wolframalpha.com";
    private String path = "/v2/query";
    private String appid = "AU7QGR-T7QT7QVGKY";
	...
	// A user would never call this ctor with null, but other ctors might.
	// This allows us to provide
	// ctors that take, say, an appid and an HTTPProvider, but users can
	// pass null for the HTTPProvider
	// if they want the default.
	this.http = (http != null) ? http : HttpProviderFactory
		.getDefaultHttpProvider();
	if (downloadDir != null) {
    ...
    // Advanced users only.
    public WAEngine(String appid, String server, String path,
	    HttpProvider http, File downloadDir) {
    ...
    public WAEngine(WAQueryParameters params, HttpProvider http,
	    File downloadDir) {
	super(params);
	this.http = http;
    }
    ...
    // Parse from a URL, either
    // http://api.wolframalpha.com/....?input=foo&appid=bar... or just
    // input=foo&appid=bar...
	...
	return new WAQueryResultImpl(query, fetcher.getBytes(), http,
		downloadDir);
    }
	...
	if (fetcher.getException() != null)
	    throw new WAException(fetcher.getException());
	return new WAQueryResultImpl(null, fetcher.getBytes(), http,
		downloadDir);
    }
    ...
    // Users can of course call HttpProviderFactory.getDefaultHttpProvider()
    // directly, but it's convenient to have
