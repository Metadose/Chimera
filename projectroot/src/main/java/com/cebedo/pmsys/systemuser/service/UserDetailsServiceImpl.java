...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username)
		...
		try {
			...
			// Spring's User
			SystemUser sysUser = userDAO.searchDatabase(username);

			// Populate the Spring User object with details from the sysUser
			// Here we just pass the username, password, and access level
			// getAuthorities() will translate the access level to the correct
			// role type

			// TODO Provide values for other fields.
			user = new User(sysUser.getUsername(), sysUser.getPassword()
					.toLowerCase(), true, true, true, true,
					getAuthorities(sysUser.getAccess()));
		} catch (Exception e) {
	 ...
	 * whether it's for a regular user or admin or others.
	 * 
	 * @param access
