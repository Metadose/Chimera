...
public class PhotoController {
    ...
    public String create(@ModelAttribute(ATTR_PHOTO) Photo photo, RedirectAttributes redirectAttrs) {
	AlertBoxGenerator alertFactory = AlertBoxGenerator.SUCCESS;
	if (photo.getId() == 0) {
	    // TODO
	    // this.photoService.create(photo);
	} else {
	    alertFactory.setMessage("Successfully <b>updated</b> the photo <b>" + photo.getName()
		    + "</b>.");
	    this.photoService.update(photo);
	}
	redirectAttrs.addFlashAttribute(SystemConstants.UI_PARAM_ALERT, alertFactory.generateHTML());
	return SystemConstants.CONTROLLER_REDIRECT + ATTR_PHOTO + "/" + SystemConstants.REQUEST_LIST;
    }

    @RequestMapping(value = SystemConstants.REQUEST_EDIT + "/" + SystemConstants.FROM + "/"
	    + SystemConstants.ORIGIN)
    ...
    @PreAuthorize("hasRole('" + SecurityRole.ROLE_PHOTO_EDITOR + "')")
    @RequestMapping(value = SystemConstants.REQUEST_EDIT + "/{" + Photo.COLUMN_PRIMARY_KEY + "}")
    public String editPhoto(@PathVariable(Photo.COLUMN_PRIMARY_KEY) int id, Model model) {
	if (id == 0) {
	    model.addAttribute(ATTR_PHOTO, new Photo());
	    return JSP_EDIT;
	}
